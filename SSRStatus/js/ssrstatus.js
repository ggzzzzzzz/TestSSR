...
function uptime() {
	$.getJSON("json/stats.json", function(result) {
		...
		for (var i = 0; i < result.servers.length; i++) {
			...
			// Latency
			TableRow.children["dl_size"].innerHTML = result.servers[i].dl_size;
			

			// Time
			TableRow.children["qrcode"].innerHTML = result.servers[i].qrcode;
			...
			// ssrurl
			//TableRow.children["ssrurl"].innerHTML = result.servers[i].ssrurl;
		...
		if (!error) {
			$("#servers > tr.accordion-toggle").each(function(i) {
				...
				var ExpandRow = $("#servers #rt" + i);
				TableRow.children["name"].children[0].children[0].className = "progress-bar progress-bar-error";
				TableRow.children["name"].children[0].children[0].innerHTML = "<small>错误</small>";
				TableRow.children["dl_size"].innerHTML = "<div class=\"progress progress-striped active\"><div style=\"width: 100%;\" class=\"progress-bar progress-bar-error\"><small>错误</small></div></div>";
				TableRow.children["qrcode"].innerHTML = "<div class=\"progress progress-striped active\"><div style=\"width: 100%;\" class=\"progress-bar progress-bar-error\"><small>错误</small></div></div>";
				...
				//TableRow.children["ssrurl"].innerHTML = "<div class=\"progress progress-striped active\"><div style=\"width: 30%;\" class=\"progress-bar progress-bar-error\"><small>错误</small></div></div>";
